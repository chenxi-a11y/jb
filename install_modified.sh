...
log_step "Creating komari system user (if not exists)..."
if ! id -u komari >/dev/null 2>&1; then
    useradd --system --no-create-home --shell /usr/sbin/nologin komari
    log_success "System user 'komari' created"
else
    log_info "User 'komari' already exists"
fi

# 设置目录权限
chown -R komari:komari "$target_dir"
